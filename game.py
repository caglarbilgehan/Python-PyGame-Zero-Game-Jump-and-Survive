...
class Game:
    ...
    def draw_game_screen(self, hero):
        ...
        screen.draw.text(f"Score: {hero.score}", topright=(self.WIDTH - 20, 15), fontsize=30, color="white")
        hero.draw()
    ...
    def draw_health_bar(self, hero):
        ...
        screen.draw.text(f"HEALTH: {hero.health}/{hero.max_health}", center=(self.WIDTH // 2, bar_y + bar_height // 2), fontsize=20, color="black")
...
class Hero:
    def __init__(self):
        ...
        self.idle_anim = ["player_idle_1", "player_idle_2", "player_idle_3"]  # Frames for idle animation
        self.current_frame = 0
        self.frame_counter = 0
        self.frame_time = 10
        self.footstep_counter = 0  # Timer for footstep sound
        self.footstep_time = 14   # How many ms intervals should the footstep sound be played?
        self.jump_left_image = "player_jump_left"
        self.jump_right_image = "player_jump_right"
        self.jump_idle_image = "player_jump"
        self.score = 0
        self.idle_timer = 0 # Timer for hero idle time
        self.idle_time_threshold = 1  # Time threshold for idle animation (e.g. 60 frames)
    ...
    def move_logic(self):
        if keyboard.left or keyboard.a:
            ...
            if not self.is_jumping:
                self.play_footstep()
            self.idle_timer = 0  # Idle timer resets when hero moves
    ...
    def jump_logic(self):
        if self.is_jumping:
            ...
            if self.actor.y >= self.ground_y:
                ...
                if game.music_on:
                    sounds.jump_end.play()  # Play falling sound
            ...
            if game.music_on:
                sounds.jump_start.play()  # Play jumping sound
...
class Spider:
    def __init__(self, side):
        ...
        self.current_frame = 0
        self.frame_counter = 0
        self.frame_time = 10  # Animation speed
...
# Creating a spider with a timer
...
class Menu:
    def __init__(self):
        ...
        self.menu_text_play = "1. Start Game"
        self.buttons = {
    ...
    def draw(self):
        if self.how_to_play_active:
            ...
            if self.game_over:
                screen.draw.text("GAME OVER!", center=(game.WIDTH // 2, game.HEIGHT // 4 - 50), fontsize=60, color="red")

            if self.win_active:
                screen.draw.text("CONGRATULATIONS! YOU WIN!", center=(game.WIDTH // 2, game.HEIGHT // 4 - 50), fontsize=60, color="green")

            title_width = game.WIDTH // 2
            title_height = game.HEIGHT // 4
            screen.draw.text("Jump and Survive!", center=(title_width, title_height), fontsize=60, color="white")
            screen.draw.text("Platform Game", center=(title_width, title_height + 40), fontsize=40, color="white")

            for name, rect in self.buttons.items():
                color = "white"
                text = ""
                if name == "play":
                    text = "1. Start Game" if self.game_over or self.win_active else self.menu_text_play
            ...
            screen.draw.text("Developer: E.Çağlar Bilgehan", center=(game.WIDTH // 2, game.HEIGHT - 100), fontsize=30, color="white")

    def draw_how_to_play(self):
        screen.fill("black")
        screen.draw.text("How to Play?", center=(game.WIDTH // 2, game.HEIGHT // 6), fontsize=60, color="white")
        instructions = [
            "Use the ARROW KEYS or A/D keys to move.",
            "Press the SPACE key to jump.",
            "Avoid enemies and earn points.",
            "Earn 1 point when an enemy completely leaves the game area!",
        ...
        screen.draw.text("Press ESC to return to the Main Menu.", center=(game.WIDTH // 2, game.HEIGHT - 50), fontsize=30, color="yellow")
        ...
        for name, rect in self.buttons.items():
            if rect.collidepoint(pos):
                if name == "play":
                    ...
                    self.active = False
                    self.menu_text_play = "1. Continue"
                elif name == "how_to_play":
...
def update():
    ...
    # Hero and spider collision control
    check_collision(hero, spiders)
    # Hero and bee collision control
...
def check_collision(hero, enemies):
    for enemy in enemies:
        if hero.actor.colliderect(enemy.actor):
            sounds.hit.play()
            hero.health -= 10
            # Don't let the hero's health drop below zero
            if hero.health < 0:
                hero.health = 0
            # Delete spider after collision
...
# Update mouse click event
def on_mouse_down(pos):
    if menu.active:
        menu.handle_mouse_down(pos)

# Update keyboard key event
def on_key_down(key):
    if menu.active:
